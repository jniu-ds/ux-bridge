...
function buildFigmaImportUserText({ prompt, projectName, pageName, figmaImport }) {
  ...
  return [
    ...
    "Treat Figma spacing as measured data, not an aesthetic suggestion. Use nodeTree layout.padding*, layout.itemSpacing, layout.counterAxisSpacing, box.relativeX/relativeY, and parentRelativeBox.parentX/parentY to reproduce exact margins, padding, gaps, and vertical rhythm at the native width.",
    "Do not add extra generous whitespace, default browser margins, or guessed section padding. Reset body margin to 0 and translate measured Figma spacing into px values before making responsive adjustments.",
    "When converting fixed positions into flex or grid, preserve the original measured gaps and offsets first. Responsive rules may scale or wrap layout, but they should not change the source spacing at the imported width.",
    ...
    "Use assets with kind=\"svg-icon\" for vector icons, logos, status symbols, and tab/navigation glyphs. Do not redraw SVG icons with plain boxes, emoji, or generic CSS shapes when a matching SVG asset URL exists.",
