(function(){let e=`ux-bridge-project-comment-summary`,t=`ux-bridge-project-thumbnail-renders`,n=1440*60*1e3,r=1440,i=1047,a=document.querySelector(`[data-projects-root]`),o=document.querySelector(`[data-projects-hero-actions]`),s=document.createElement(`div`),c=document.createElement(`div`),l=document.createElement(`div`),u=0,d=0,f=0,p=null;if(!a)return;s.className=`bridge-projects__thumbnail-generator`,s.setAttribute(`aria-hidden`,`true`),document.body.append(s),c.className=`bridge-projects__menu-host`,document.body.append(c),l.className=`bridge-projects__modal-host`,document.body.append(l);let m={loading:!0,projects:[],canCreateProjects:!1,canDuplicateProjects:!1,status:``,tone:`neutral`,toastClosing:!1,creating:!1,commentSummaryByProject:new Map,openMenuProjectId:``,openMenuPosition:null,ownerPickerProjectId:``,ownerPickerPosition:null,ownerPickerSearchQuery:``,updatingOwnerProjectId:``,editingProjectId:``,renamingProjectId:``,pendingProjectNameById:{},pendingProjectDescriptionById:{},dirtyProjectById:{},deleteModalProjectId:``,deletingProjectId:``,exitingProjectId:``,highlightedProjectId:``,summaryRefreshTimer:0,summaryRequestToken:0,thumbnailActiveProjectId:``,thumbnailRequestToken:``,thumbnailFailures:new Set,thumbnailRefreshStarted:!1,loadedThumbnailDataByProject:new Map,recentProjectOpenById:{},sortKey:`recentOpen`,sortDirection:`desc`,sortPresetMenuOpen:!1,searchQuery:``,availableUsers:[],selectedProjectIds:[],bulkDuplicatingProjects:!1,bulkDeleteProjectIds:[],bulkDeletingProjects:!1};function h(){if(window.uxBridgeUser?.email)return window.uxBridgeUser;try{let e=sessionStorage.getItem(`ux-bridge-user`);return e?JSON.parse(e):null}catch{return null}}function g(){return`ux-bridge-project-recent-open:${String(h()?.email||`anonymous`).trim().toLowerCase()}`}function _(e,t){return`ux-bridge-comments-seen:${String(h()?.email||`anonymous`).trim().toLowerCase()}:${e}:${t}`}function v(e,t){try{let n=localStorage.getItem(_(e,t));if(!n)return{lastSeenCommentId:``,lastSeenAt:0};let r=JSON.parse(n);return r&&typeof r==`object`?{lastSeenCommentId:String(r.lastSeenCommentId||``).trim(),lastSeenAt:Number(r.lastSeenAt)||0}:{lastSeenCommentId:String(n||``).trim(),lastSeenAt:0}}catch{return{lastSeenCommentId:``,lastSeenAt:0}}}function y(){try{let e=localStorage.getItem(g());if(!e)return{};let t=JSON.parse(e);return t&&typeof t==`object`?t:{}}catch{return{}}}function b(e){try{localStorage.setItem(g(),JSON.stringify(e))}catch{}}function ee(e=``){return Number(m.recentProjectOpenById?.[e])||0}function x(e={}){return!e||typeof e!=`object`?{}:Object.fromEntries(Object.entries(e).map(([e,t])=>[String(e||``).trim(),Number(t)||0]).filter(([e,t])=>e&&t>0))}function S(...e){let t={};return e.forEach(e=>{let n=x(e);Object.entries(n).forEach(([e,n])=>{t[e]=Math.max(Number(t[e])||0,Number(n)||0)})}),t}function te(e={},t={}){let n=x(e),r=x(t),i=Object.keys(n),a=Object.keys(r);return i.length===a.length?i.every(e=>Number(n[e])===Number(r[e])):!1}function C(e={}){let t=x(e);m.recentProjectOpenById=t,b(t)}function ne(e={},{useBeacon:t=!1}={}){let n=x(e);if(!Object.keys(n).length)return;let r=JSON.stringify({action:`syncRecentOpenMap`,recentProjectOpenById:n});if(t&&navigator.sendBeacon){let e=new Blob([r],{type:`application/json`});navigator.sendBeacon(`/api/projects`,e);return}fetch(`/api/projects`,{method:`POST`,credentials:`include`,headers:{"Content-Type":`application/json`},body:r,keepalive:t}).catch(()=>{})}function re(e=``,t=Date.now(),{useBeacon:n=!1}={}){let r=String(e||``).trim();if(!r)return;let i=JSON.stringify({action:`markRecentOpen`,project:r,openedAt:t});if(n&&navigator.sendBeacon){let e=new Blob([i],{type:`application/json`});navigator.sendBeacon(`/api/projects`,e);return}fetch(`/api/projects`,{method:`POST`,credentials:`include`,headers:{"Content-Type":`application/json`},body:i,keepalive:n}).catch(()=>{})}function ie(e=``,{useBeacon:t=!1}={}){if(!e)return;let n=Date.now();C(S(m.recentProjectOpenById,{[e]:n})),re(e,n,{useBeacon:t})}function w(e={}){let t=S(m.recentProjectOpenById,e.recentProjectOpenById);te(t,m.recentProjectOpenById)||C(t)}function T(){return{totalComments:0,unreadComments:0,unreadMentions:0}}function ae(e){let t=D(h()?.email);return!e||!t?!1:t===D(e.ownerEmail)?!0:(Array.isArray(e.projectMembers)?e.projectMembers:[]).some(e=>{let n=D(e?.email),r=String(e?.status||``).trim().toLowerCase();return n===t&&(r===`active`||r===`pending`)})}function E(e){return String(e??``).replaceAll(`&`,`&amp;`).replaceAll(`<`,`&lt;`).replaceAll(`>`,`&gt;`).replaceAll(`"`,`&quot;`)}function oe(e){return String(e??``).replace(/[.*+?^${}()|[\]\\]/g,`\\$&`)}function D(e){return String(e||``).trim().toLowerCase()}function O(e,t){let n=String(e??``),r=String(t||``).trim();if(!r)return E(n);let i=RegExp(`(${oe(r)})`,`gi`);return E(n).replace(i,`<mark class="bridge-table-search__highlight">$1</mark>`)}function se(e,t){let n=String(t||``).trim().toLowerCase();return n?[e.name,e.description,e.ownerName].map(e=>String(e||``).toLowerCase()).some(e=>e.includes(n)):!0}function ce(e,t){let n=String(t||``).trim().toLowerCase();return n?[e.fullName,e.email,e.role].map(e=>String(e||``).toLowerCase()).some(e=>e.includes(n)):!0}function le(e=null,t=null){window.requestAnimationFrame(()=>{let n=a.querySelector(`[data-project-search]`);n instanceof HTMLInputElement&&(n.focus(),typeof e==`number`&&typeof t==`number`&&n.setSelectionRange(e,t))})}function ue(e){let t=String(e||``).trim().split(/\s+/).filter(Boolean);return t.length?t.slice(0,2).map(e=>e[0]?.toUpperCase()||``).join(``):`UX`}function de(e){return e===`error`?`
    ...
    `}function k(){if(window.clearTimeout(u),window.clearTimeout(d),!m.status){m.toastClosing=!1;return}m.toastClosing=!0,J(),d=window.setTimeout(()=>{m.status=``,m.tone=`neutral`,m.toastClosing=!1,J()},260)}function fe(e=``){window.clearTimeout(f),m.highlightedProjectId=e,e&&(f=window.setTimeout(()=>{m.highlightedProjectId===e&&(m.highlightedProjectId=``,J())},2e3))}function A(){let e=new Set(m.projects.map(e=>e.id));return m.selectedProjectIds.filter(t=>e.has(t))}function pe(e=``){return A().includes(e)}function me(e,t){let n=new Set(A());t?n.add(e):n.delete(e),m.selectedProjectIds=Array.from(n)}function he(){m.selectedProjectIds=A()}function j(e=``,t=`neutral`){if(window.clearTimeout(u),window.clearTimeout(d),!e){k();return}m.status=e,m.tone=t,m.toastClosing=!1,u=window.setTimeout(()=>{k()},4e3),J()}function ge(){try{let e=localStorage.getItem(t);if(!e)return{};let n=JSON.parse(e);return n&&typeof n==`object`?n:{}}catch{return{}}}function _e(e){try{localStorage.setItem(t,JSON.stringify(e))}catch{}}function ve(e){let t=ge(),r=`${e.pathname}${e.searchParams.get(`table-thumb`)===`1`?`?table-thumb=1`:e.search}`,i=t[r],a=Date.now();if(i&&typeof i==`object`&&typeof i.token==`string`&&typeof i.createdAt==`number`&&a-i.createdAt<n)return i.token;let o=String(a);return t[r]={token:o,createdAt:a},_e(t),o}function M(e){return String(e?.thumbnailUrl||e?.launchUrl||``).trim()}function N(e){return!!String(e?.thumbnailDataUrl||``).trim()}function ye(e){let t=String(e?.thumbnailDataUrl||``).trim(),r=Number(e?.thumbnailUpdatedAt)||0,i=String(e?.thumbnailSourceUrl||``).trim(),a=M(e);return!!(t&&r&&i===a&&Date.now()-r<n)}function be(e){return!e||m.thumbnailFailures.has(e.id)?!1:!ye(e)}function xe(){return p?.isConnected?p:(p=document.createElement(`iframe`),p.className=`bridge-projects__thumbnail-generator-frame`,p.setAttribute(`tabindex`,`-1`),s.style.width=`${r}px`,s.style.height=`${i}px`,p.style.width=`${r}px`,p.style.height=`${i}px`,s.replaceChildren(p),p)}function Se(e,t){let n=M(e);if(!n)return``;let r=new URL(n,window.location.origin),i=ve(r);return r.searchParams.set(`project`,e.id),r.searchParams.set(`thumb-capture`,`1`),r.searchParams.set(`thumb-request`,t),r.searchParams.set(`thumb-load`,i),`${r.pathname}${r.search}`}function P(e){let t=N(e),n=m.thumbnailActiveProjectId===e.id,r=!!e?.isLocked;return`
    ...
    `}function H(){if(!m.ownerPickerProjectId||!m.ownerPickerPosition){(!m.openMenuProjectId||!m.openMenuPosition)&&(c.innerHTML=``);return}let e=m.projects.find(e=>e.id===m.ownerPickerProjectId);if(!e){B();return}let t=m.availableUsers.filter(e=>ce(e,m.ownerPickerSearchQuery));c.innerHTML=`
      ...
      >
        <div class="bridge-table-search-bar bridge-projects-table__owner-picker-search">
          <label class="bridge-table-search">
            ...
            <input
              ...
              type="search"
              value="${E(m.ownerPickerSearchQuery)}"
        ...
        <div class="bridge-projects-table__owner-picker-list">
          ${t.length?t.map(t=>{let n=D(t.email)===D(e.ownerEmail),r=ue(t.fullName||t.email||`User`),i=t.avatarColor?` style="--avatar-bg:${E(t.avatarColor)}"`:``,a=t.avatarUrl?`<span class="bridge-projects-table__owner-avatar has-photo"${i}><img src="${E(t.avatarUrl)}" alt="" /></span>`:`<span class="bridge-projects-table__owner-avatar"${i}>${E(r)}</span>`;return`
                      <button
                      ...
                      >
                        ...
                        <span class="bridge-projects-table__owner-picker-copy">
                          <strong>${O(t.fullName||t.email,m.ownerPickerSearchQuery)}</strong>
                          <span>${O(t.email,m.ownerPickerSearchQuery)}</span>
                        </span>
    ...
    `}function Ne(){a.querySelectorAll(`[data-project-comments-cell]`).forEach(e=>{let t=e.getAttribute(`data-project-comments-cell`)||``;e.innerHTML=Me(m.projects.find(e=>e.id===t),m.commentSummaryByProject.get(t)||T())})}function Pe(){let e=m.sortDirection===`desc`?-1:1,t=[...m.projects];return t.sort((t,n)=>{let r=0;if(m.sortKey===`owner`)r=String(t.ownerName||``).localeCompare(String(n.ownerName||``),void 0,{sensitivity:`base`}),r||=String(t.name||``).localeCompare(String(n.name||``),void 0,{sensitivity:`base`});else if(m.sortKey===`pages`)r=Number(t.pageCount||0)-Number(n.pageCount||0),r||=String(t.name||``).localeCompare(String(n.name||``),void 0,{sensitivity:`base`});else if(m.sortKey===`comments`){let e=m.commentSummaryByProject.get(t.id)||{totalComments:0,unreadComments:0},i=m.commentSummaryByProject.get(n.id)||{totalComments:0,unreadComments:0};r=Number(e.totalComments||0)-Number(i.totalComments||0),r||=Number(e.unreadComments||0)-Number(i.unreadComments||0),r||=String(t.name||``).localeCompare(String(n.name||``),void 0,{sensitivity:`base`})}else m.sortKey===`createdAt`?(r=Number(t.createdAt||0)-Number(n.createdAt||0),r||=String(t.name||``).localeCompare(String(n.name||``),void 0,{sensitivity:`base`})):m.sortKey===`recentOpen`?(r=ee(t.id)-ee(n.id),r||=Number(t.createdAt||0)-Number(n.createdAt||0),r||=String(t.name||``).localeCompare(String(n.name||``),void 0,{sensitivity:`base`})):(r=String(t.name||``).localeCompare(String(n.name||``),void 0,{sensitivity:`base`}),r||=String(t.ownerName||``).localeCompare(String(n.ownerName||``),void 0,{sensitivity:`base`}));return r*e}),t}function q(e){return m.sortKey===e?m.sortDirection===`desc`?`
    ...
    `:`
      ...
      `}function J(){let e=m.status?`
          <div class="bridge-projects__toast${m.tone===`error`?` bridge-projects__toast--error`:``}${m.toastClosing?` is-closing`:``}" role="status" aria-live="polite">
            ${de(m.tone)}
            <span class="bridge-projects__toast-message">${E(m.status)}</span>
        ...
        `:``;o&&(o.innerHTML=t);let n=(m.loading?[]:Pe()).filter(e=>se(e,m.searchQuery)),r=!m.loading&&m.projects.length>5,i=m.canDuplicateProjects||m.projects.some(e=>!!e.canManageIdentity),s=i&&!m.loading&&m.projects.length>5,c=!m.loading&&m.projects.length>5,l=A(),u=l.length,d=n.map(e=>e.id),f=d.length?d.every(e=>l.includes(e)):!1,p=r?`
        ...
        `:p,_=A().map(e=>m.projects.find(t=>t.id===e)).filter(Boolean),v=_.length>0&&_.every(e=>!!e.canManageIdentity),y=s&&u?`
        ...
        `:n.length?n.map(e=>{let t=m.commentSummaryByProject.get(e.id)||T(),n=ue(e.ownerName||`Unknown`),r=e.ownerAvatarColor?` style="--avatar-bg:${E(e.ownerAvatarColor)}"`:``,a=e.ownerAvatarUrl?`<span class="bridge-projects-table__owner-avatar has-photo"${r}><img src="${E(e.ownerAvatarUrl)}" alt="" /></span>`:`<span class="bridge-projects-table__owner-avatar" aria-hidden="true"${r}>${E(n)}</span>`,o=!!e.canManageIdentity,c=o&&m.availableUsers.length>0,l=m.canDuplicateProjects&&e.kind===`dynamic`,u=o||l,d=N(e),f=m.thumbnailActiveProjectId===e.id,p=m.editingProjectId===e.id,h=m.renamingProjectId===e.id;String(m.pendingProjectNameById[e.id]||e.name||``).trim();let g=String(m.pendingProjectDescriptionById[e.id]??e.description??``),_=!!m.dirtyProjectById[e.id];return`
              <tr
                    ...
                    <div class="bridge-projects-table__identity">
                              ...
                              <div class="bridge-projects-table__rename-fields">
                                <div class="bridge-projects-table__field-wrap">
                                  <input
                                    ...
                                    type="text"
                                    value="${E(m.pendingProjectNameById[e.id]||e.name)}"
                      ...
                      ${p?``:`
                            <div class="bridge-projects-table__identity-top">
                                <strong>${O(e.name,m.searchQuery)}</strong>
                              ${o?`
                                      <button
                            ...
                            <span>${O(e.description||`A project inside UX Bridge.`,m.searchQuery)}</span>
                          `}
                    </div>
                  </div>
                </td>
                <td class="bridge-projects-table__owner-cell">
                  <div class="bridge-projects-table__owner-control">
                    <span class="bridge-projects-table__owner-pill">
                      ${a}
                      <span>${O(e.ownerName||`Unknown`,m.searchQuery)}</span>
                    </span>
                    ${c?`
                          <button
      ...
      ${m.loading?b:`
        ...
        `}
    `,U(),R()}function Y(e){!e||!e.id||(m.projects.findIndex(t=>t.id===e.id)>=0?m.projects=m.projects.map(t=>t.id===e.id?e:t):m.projects=[e,...m.projects],m.commentSummaryByProject.has(e.id)||m.commentSummaryByProject.set(e.id,T()))}function Fe(e){let t=m.projects.find(t=>t.id===e);if(!t)return;z(),m.pendingProjectNameById[e]=t.name||``,m.pendingProjectDescriptionById[e]=t.description||``,m.dirtyProjectById[e]=!1,m.editingProjectId=e,J();let n=a.querySelector(`[data-project-rename-input="${e}"]`);n instanceof HTMLInputElement&&window.requestAnimationFrame(()=>{n.focus(),n.select()})}async function X({background:e=!1}={}){e||(m.loading=!0,J());let t=await fetch(`/api/projects`,{credentials:`include`,cache:`no-store`}),n=await t.json().catch(()=>({}));if(!t.ok||!n?.ok)throw Error(n?.error||`Unable to load projects.`);m.projects=Array.isArray(n.projects)?n.projects:[],m.availableUsers=Array.isArray(n.availableUsers)?n.availableUsers:[];let r=x(n.recentProjectOpenById),i=S(r,y());C(i),te(i,r)||ne(i),he(),m.canCreateProjects=!!n.canCreateProjects,m.canDuplicateProjects=!!n.canDuplicateProjects,m.editingProjectId&&!m.projects.some(e=>e.id===m.editingProjectId)&&W(),m.ownerPickerProjectId&&!m.projects.some(e=>e.id===m.ownerPickerProjectId)&&B(),m.commentSummaryByProject=L(m.projects)||new Map(m.projects.map(e=>[e.id,T()])),m.loading=!1,J();try{let e=sessionStorage.getItem(`ux-bridge-share-toast`);e&&(sessionStorage.removeItem(`ux-bridge-share-toast`),j(e))}catch{}if(Re({immediate:!L(m.projects)}),m.thumbnailRefreshStarted=!0,m.thumbnailFailures.clear(),Q(),m.editingProjectId){let e=a.querySelector(`[data-project-rename-input="${m.editingProjectId}"]`);e instanceof HTMLInputElement&&window.requestAnimationFrame(()=>{e.focus(),e.select()})}}async function Ie(e,t=0){let n=e.map(e=>({project:e.id,pages:[...e.hasOverview?[`overview`]:[],...(Array.isArray(e.pages)?e.pages:[]).map(e=>String(e.id||``).trim()).filter(Boolean)]})).filter(e=>e.project&&e.pages.length);if(!n.length){let t=new Map(e.map(e=>[e.id,T()]));return we(e,t),t}try{let r=new URLSearchParams({summary:`projects`,projects:JSON.stringify(n)}),i=await fetch(`/api/comments?${r.toString()}`,{credentials:`include`,cache:`no-store`}),a=await i.json().catch(()=>({}));if(!i.ok||!a?.ok||!Array.isArray(a.projects))throw Error(`Unable to load project comment summaries.`);let o=new Map(e.map(e=>[e.id,T()]));return a.projects.forEach(t=>{let n=String(t?.project||``).trim(),r=e.find(e=>e.id===n);if(!n||!r)return;let i=(Array.isArray(t.summary)?t.summary:[]).reduce((e,t)=>{let n=String(t.page||``).trim(),i=v(r.id,n),a=Number(t.lastSeenAt||0),o=String(t.lastSeenCommentId||``)||i.lastSeenCommentId,s=e=>{if(!Array.isArray(e)||!e.length)return 0;if(!o)return e.length;let t=e.findIndex(e=>e===o);return t<0?e.length:Math.max(e.length-(t+1),0)};return e.totalComments+=Number(t.count||0),e.unreadComments+=a>0?Number(t.unreadCount||0):s(Array.isArray(t.commentIds)?t.commentIds:[]),e.unreadMentions+=a>0?Number(t.unreadMentionCount||0):s(Array.isArray(t.mentionCommentIds)?t.mentionCommentIds:[]),e},T());o.set(n,i)}),t&&t!==m.summaryRequestToken?null:(we(e,o),o)}catch{return new Map(e.map(e=>[e.id,T()]))}}async function Le(){if(m.loading||document.visibilityState!==`visible`)return;let e=Date.now();m.summaryRequestToken=e;let t=await Ie(m.projects,e);if(!(!t||m.summaryRequestToken!==e)){if(m.commentSummaryByProject=t,m.loading||m.sortKey===`comments`){J();return}Ne()}}function Re({immediate:e=!1}={}){Te();let t=()=>{Le().catch(()=>{}),m.summaryRefreshTimer=window.setTimeout(t,3e3)};if(e){t();return}m.summaryRefreshTimer=window.setTimeout(t,1200)}async function ze(e,t,n,r=`auto`){let i=await fetch(`/api/projects`,{method:`POST`,credentials:`include`,headers:{"Content-Type":`application/json`},body:JSON.stringify({action:`updateThumbnail`,project:e,sourceUrl:t,imageDataUrl:n,mode:r})}),a=await i.json().catch(()=>({}));if(!i.ok||!a?.ok||!a?.project)throw Error(a?.error||`Unable to update project thumbnail.`);return a.project}function Z(e=``){m.thumbnailActiveProjectId=``,m.thumbnailRequestToken=``,e&&F([e])}function Q(e=``,t=`auto`){if(m.thumbnailActiveProjectId||document.visibilityState!==`visible`)return;let n=e?m.projects.find(t=>t.id===e):m.projects.find(e=>be(e));if(!n)return;let r=`${Date.now()}-${n.id}`;m.thumbnailActiveProjectId=n.id,m.thumbnailRequestToken=r,F([n.id]);let i=xe(),a=async i=>{if(i.origin!==window.location.origin)return;let o=i.data;if(!(!o||o.requestToken!==r||o.projectId!==n.id)){if(o.type===`uxbridge:thumbnail-capture-failed`){window.removeEventListener(`message`,a),m.thumbnailFailures.add(n.id),Z(n.id),Q();return}if(!(o.type!==`uxbridge:thumbnail-captured`||!o.imageDataUrl)){window.removeEventListener(`message`,a);try{let e=await ze(n.id,o.sourceUrl,o.imageDataUrl,t);m.loadedThumbnailDataByProject.delete(e.id),m.projects=m.projects.map(t=>t.id===e.id?e:t),F([e.id])}catch{m.thumbnailFailures.add(n.id)}finally{Z(n.id),e||Q()}}}};window.addEventListener(`message`,a),i.src=Se(n,r)}async function Be(){if(!m.creating){m.creating=!0,J();try{let e=await fetch(`/api/projects`,{method:`POST`,credentials:`include`,headers:{"Content-Type":`application/json`},body:JSON.stringify({action:`createProject`})}),t=await e.json().catch(()=>({}));if(!e.ok||!t?.ok)throw Error(t?.error||`Unable to create project.`);m.creating=!1,w(t),Y(t.project),fe(t.project?.id||``),j(`Created ${t.project?.name||`new project`}.`),J(),X({background:!0}).catch(()=>{})}catch(e){m.creating=!1,j(e instanceof Error?e.message:`Unable to create project.`,`error`)}}}o&&o.addEventListener(`click`,async e=>{e.target.closest(`[data-create-project]`)&&await Be()});async function Ve(e){if(e.target.closest(`[data-dismiss-status]`)){k();return}if(e.target.closest(`[data-create-project]`)){await Be();return}if(e.target.closest(`[data-project-sort-toggle]`)){e.preventDefault(),e.stopPropagation(),m.sortPresetMenuOpen=!m.sortPresetMenuOpen,J();return}let t=e.target.closest(`[data-project-sort-option]`);if(t){e.preventDefault(),e.stopPropagation(),je(t.getAttribute(`data-project-sort-option`)||`recentOpen`),J();return}let n=e.target.closest(`[data-sort-key]`);if(n){V();let e=n.getAttribute(`data-sort-key`)||``;if(!e)return;m.sortKey===e?(m.sortDirection===`asc`||(m.sortKey=`recentOpen`),m.sortDirection=`desc`):(m.sortKey=e,m.sortDirection=`asc`),J();return}let r=e.target.closest(`[data-project-thumbnail-refresh]`);if(r){e.preventDefault(),e.stopPropagation();let t=r.getAttribute(`data-project-thumbnail-refresh`)||``;if(!t||m.thumbnailActiveProjectId)return;m.thumbnailFailures.delete(t),Q(t,`manual`);return}let i=e.target.closest(`[data-project-request-access]`);if(i){e.preventDefault(),e.stopPropagation();let t=i.getAttribute(`data-project-request-access`)||``;if(!t)return;try{let e=await fetch(`/api/projects`,{method:`POST`,credentials:`include`,headers:{"Content-Type":`application/json`},body:JSON.stringify({action:`requestAccess`,project:t})}),n=await e.json().catch(()=>({}));if(!e.ok||!n?.ok)throw Error(n?.error||`Unable to request project access.`);j(n.message||`Access request sent.`),await X()}catch(e){j(e instanceof Error?e.message:`Unable to request project access.`,`error`)}return}let o=e.target.closest(`[data-project-menu-toggle]`);if(o){W();let e=o.getAttribute(`data-project-menu-toggle`)||``;m.openMenuProjectId===e?z():Ee(o,e);return}let s=e.target.closest(`[data-project-owner-edit]`);if(s){e.preventDefault(),e.stopPropagation(),W();let t=s.getAttribute(`data-project-owner-edit`)||``;m.ownerPickerProjectId===t?B():Oe(s,t);return}if(e.target.closest(`[data-project-bulk-duplicate]`)){e.preventDefault(),e.stopPropagation();let t=A();if(!t.length)return;m.bulkDuplicatingProjects=!0,J();try{for(let e of t){let t=await fetch(`/api/projects`,{method:`POST`,credentials:`include`,headers:{"Content-Type":`application/json`},body:JSON.stringify({action:`duplicateProject`,project:e})}),n=await t.json().catch(()=>({}));if(!t.ok||!n?.ok)throw Error(n?.error||`Unable to duplicate selected projects.`);w(n),Y(n.project)}m.bulkDuplicatingProjects=!1,m.selectedProjectIds=[],j(`Duplicated ${t.length} ${t.length===1?`project`:`projects`}.`),J(),X({background:!0}).catch(()=>{})}catch(e){m.bulkDuplicatingProjects=!1,j(e instanceof Error?e.message:`Unable to duplicate selected projects.`,`error`),J()}return}if(e.target.closest(`[data-project-bulk-delete]`)){e.preventDefault(),e.stopPropagation();let t=A();if(!t.length)return;m.bulkDeleteProjectIds=t,U();return}let c=e.target.closest(`[data-project-rename]`);if(c){e.preventDefault(),e.stopPropagation(),Fe(c.getAttribute(`data-project-rename`)||``);return}let l=e.target.closest(`[data-project-edit-trigger]`);if(l){e.preventDefault(),e.stopPropagation(),Fe(l.getAttribute(`data-project-edit-trigger`)||``);return}let u=e.target.closest(`[data-project-rename-clear]`);if(u){e.preventDefault(),e.stopPropagation();let t=u.getAttribute(`data-project-rename-clear`)||``,n=a.querySelector(`[data-project-rename-input="${t}"]`);n instanceof HTMLInputElement&&(n.value=``,m.pendingProjectNameById[t]=``,m.dirtyProjectById[t]=!0,G(t),n.focus());return}let d=e.target.closest(`[data-project-description-clear]`);if(d){e.preventDefault(),e.stopPropagation();let t=d.getAttribute(`data-project-description-clear`)||``,n=a.querySelector(`[data-project-description-input="${t}"]`);n instanceof HTMLTextAreaElement&&(n.value=``,m.pendingProjectDescriptionById[t]=``,m.dirtyProjectById[t]=!0,G(t),n.focus());return}let f=e.target.closest(`[data-project-rename-save]`);if(f){e.preventDefault(),e.stopPropagation();let t=f.getAttribute(`data-project-rename-save`)||``,n=m.projects.find(e=>e.id===t),r=a.querySelector(`[data-project-rename-input="${t}"]`),i=a.querySelector(`[data-project-description-input="${t}"]`),o=String(r instanceof HTMLInputElement?r.value:m.pendingProjectNameById[t]||``).trim(),s=String(i instanceof HTMLTextAreaElement?i.value:m.pendingProjectDescriptionById[t]||``).trim();if(!t||!n)return;if(!o){j(`Project name is required.`,`error`),r instanceof HTMLInputElement&&r.focus();return}if(o===String(n.name||``).trim()&&s===String(n.description||``).trim()){W(),J();return}m.renamingProjectId=t,J();try{let e=await fetch(`/api/projects`,{method:`POST`,credentials:`include`,headers:{"Content-Type":`application/json`},body:JSON.stringify({action:`renameProject`,project:t,name:o,description:s})}),n=await e.json().catch(()=>({}));if(!e.ok||!n?.ok||!n?.project)throw Error(n?.error||`Unable to rename project.`);m.projects=m.projects.map(e=>e.id===t?n.project:e),W(),m.renamingProjectId=``,j(`Renamed project to ${n.project.name}.`),J()}catch(e){m.renamingProjectId=``,j(e instanceof Error?e.message:`Unable to rename project.`,`error`),J()}return}let p=e.target.closest(`[data-project-duplicate]`);if(p){let e=p.getAttribute(`data-project-duplicate`)||``;z();try{let t=await fetch(`/api/projects`,{method:`POST`,credentials:`include`,headers:{"Content-Type":`application/json`},body:JSON.stringify({action:`duplicateProject`,project:e})}),n=await t.json().catch(()=>({}));if(!t.ok||!n?.ok)throw Error(n?.error||`Unable to duplicate project.`);w(n),Y(n.project),j(`Duplicated ${n.project?.name||`project`}.`),J(),X({background:!0}).catch(()=>{})}catch(e){j(e instanceof Error?e.message:`Unable to duplicate project.`,`error`)}return}let h=e.target.closest(`[data-project-delete]`);if(h){let e=h.getAttribute(`data-project-delete`)||``;if(!m.projects.find(t=>t.id===e))return;z(),m.bulkDeleteProjectIds=[e],U();return}let g=e.target.closest(`[data-project-owner-option]`);if(g){e.preventDefault(),e.stopPropagation();let t=g.getAttribute(`data-project-owner-option`)||``,n=g.getAttribute(`data-owner-email`)||``,r=m.projects.find(e=>e.id===t);if(!t||!n||!r)return;m.updatingOwnerProjectId=t,H();try{let e=await fetch(`/api/projects`,{method:`POST`,credentials:`include`,headers:{"Content-Type":`application/json`},body:JSON.stringify({action:`updateProjectOwner`,project:t,ownerEmail:n})}),r=await e.json().catch(()=>({}));if(!e.ok||!r?.ok||!r?.project)throw Error(r?.error||`Unable to update project owner.`);m.projects=m.projects.map(e=>e.id===t?r.project:e),m.updatingOwnerProjectId=``,B(),j(`Updated owner for ${r.project.name}.`),J()}catch(e){m.updatingOwnerProjectId=``,j(e instanceof Error?e.message:`Unable to update project owner.`,`error`),H()}return}if(e.target.closest(`[data-project-owner-picker]`))return;let _=e.target.closest(`[data-project-launch-url]`);if(!_){V(),z(),B();return}if(e.target.closest(`[data-project-actions]`)||e.target.closest(`[data-project-rename-editor]`)||e.target.closest(`.bridge-table-select`)||e.target.closest(`[data-project-thumbnail-refresh]`)||_.dataset.projectLocked===`true`||m.editingProjectId)return;let v=_.getAttribute(`data-project-launch-url`),y=_.getAttribute(`data-project-id`)||``;v&&(ie(y,{useBeacon:!0}),window.location.href=v)}a.addEventListener(`click`,Ve),c.addEventListener(`click`,Ve),a.addEventListener(`change`,e=>{let t=e.target.closest(`[data-project-select]`);if(t instanceof HTMLInputElement){me(t.getAttribute(`data-project-select`)||``,t.checked),J();return}let n=e.target.closest(`[data-project-select-all]`);if(n instanceof HTMLInputElement){let e=Pe().filter(e=>se(e,m.searchQuery)).map(e=>e.id),t=new Set(A());n.checked?e.forEach(e=>t.add(e)):e.forEach(e=>t.delete(e)),m.selectedProjectIds=Array.from(t),J()}}),l.addEventListener(`click`,async e=>{if(e.target.closest(`[data-confirm-project-delete]`)){let e=Array.isArray(m.bulkDeleteProjectIds)?m.bulkDeleteProjectIds.filter(Boolean):[],t=e.map(e=>m.projects.find(t=>t.id===e)).filter(Boolean);if(!t.length)return;m.bulkDeletingProjects=!0,U();try{for(let e of t){let t=await fetch(`/api/projects`,{method:`POST`,credentials:`include`,headers:{"Content-Type":`application/json`},body:JSON.stringify({action:`deleteProject`,project:e.id})}),n=await t.json().catch(()=>({}));if(!t.ok||!n?.ok)throw Error(n?.error||`Unable to delete project.`)}let n=new Set(e);m.bulkDeleteProjectIds=[],m.bulkDeletingProjects=!1,m.projects=m.projects.filter(e=>!n.has(e.id)),m.selectedProjectIds=A().filter(e=>!n.has(e)),j(`Deleted ${t.length} ${t.length===1?`project`:`projects`}.`),J(),X({background:!0}).catch(()=>{})}catch(e){m.bulkDeletingProjects=!1,j(e instanceof Error?e.message:`Unable to delete project.`,`error`),U()}return}let t=e.target.closest(`[data-close-project-delete]`);if(t||e.target.closest(`[data-project-delete-overlay]`)){if(e.target.closest(`.bridge-project-delete__modal`)&&!t)return;m.bulkDeletingProjects||(m.bulkDeleteProjectIds=[],U());return}}),a.addEventListener(`keydown`,e=>{let t=e.target.closest(`[data-project-rename-input]`);if(t){if(e.key===`Enter`){e.preventDefault();let n=t.getAttribute(`data-project-rename-input`)||``,r=a.querySelector(`[data-project-rename-save="${n}"]`);r instanceof HTMLButtonElement&&!r.disabled&&r.click()}else e.key===`Escape`&&(e.preventDefault(),W(),J());return}let n=e.target.closest(`[data-project-description-input]`);if(n){if(e.key===`Enter`){e.preventDefault();let t=n.getAttribute(`data-project-description-input`)||``,r=a.querySelector(`[data-project-rename-save="${t}"]`);r instanceof HTMLButtonElement&&!r.disabled&&r.click()}else e.key===`Escape`&&(e.preventDefault(),W(),J());return}let r=e.target.closest(`[data-project-launch-url]`);if(!r||e.target.closest(`.bridge-table-select`)||e.key!==`Enter`&&e.key!==` `||(e.preventDefault(),r.dataset.projectLocked===`true`)||m.editingProjectId)return;let i=r.getAttribute(`data-project-launch-url`);i&&(ie(r.getAttribute(`data-project-id`)||``,{useBeacon:!0}),window.location.href=i)}),a.addEventListener(`input`,e=>{let t=e.target.closest(`[data-project-search]`);if(t instanceof HTMLInputElement){let e=t.selectionStart,n=t.selectionEnd;m.searchQuery=t.value,J(),le(e,n);return}let n=e.target.closest(`[data-project-rename-input]`);if(n){let e=n.getAttribute(`data-project-rename-input`)||``;m.pendingProjectNameById[e]=n.value,m.dirtyProjectById[e]=!0,G(e);return}let r=e.target.closest(`[data-project-description-input]`);if(r){let e=r.getAttribute(`data-project-description-input`)||``;m.pendingProjectDescriptionById[e]=r.value,m.dirtyProjectById[e]=!0,G(e)}}),c.addEventListener(`input`,e=>{let t=e.target.closest(`[data-project-owner-search]`);if(t instanceof HTMLInputElement){let e=t.selectionStart,n=t.selectionEnd;m.ownerPickerSearchQuery=t.value,H(),De(e,n)}}),document.addEventListener(`click`,e=>{if(m.editingProjectId&&!e.target.closest(`[data-project-rename-editor]`)){W(),J();return}if(m.openMenuProjectId){if(e.target.closest(`[data-project-actions]`)||e.target.closest(`.bridge-projects-table__floating-menu`))return;z();return}if(m.ownerPickerProjectId){if(e.target.closest(`[data-project-owner-edit]`)||e.target.closest(`[data-project-owner-picker]`))return;B()}if(m.sortPresetMenuOpen){if(e.target.closest(`[data-project-sort]`))return;V(),J()}});let $=()=>{document.visibilityState===`visible`&&Re({immediate:!0})};document.addEventListener(`visibilitychange`,$),window.addEventListener(`pageshow`,$),window.addEventListener(`focus`,$),X().catch(e=>{m.loading=!1,j(e instanceof Error?e.message:`Unable to load projects.`,`error`)})})();