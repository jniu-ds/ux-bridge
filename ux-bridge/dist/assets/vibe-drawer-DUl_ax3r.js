(function(){let e=document.querySelector(`[data-vibe-root]`);if(!e)return;let t=`/api/projects`,n=(()=>{let e=document.querySelector(`[data-mobile-sheet-backdrop]`);if(e)return e;let t=document.createElement(`button`);return t.type=`button`,t.className=`bridge-mobile-sheet-backdrop`,t.setAttribute(`data-mobile-sheet-backdrop`,``),t.setAttribute(`aria-label`,`Close drawer`),t.hidden=!0,document.body.append(t),t})(),r=String(window.__UX_BRIDGE_CODEX_BRIDGE_URL__||`http://127.0.0.1:4318`).trim(),i=[{value:`owner_admin_only`,label:`Owner + Admin only`},{value:`contributors`,label:`Contributors can edit`},{value:`all_members`,label:`All project members`}],a=new URLSearchParams(window.location.search),o=String(a.get(`project`)||``).trim().toLowerCase(),s=String(a.get(`page`)||``).trim().toLowerCase(),c={drawerOpen:!1,loading:!1,applying:!1,restoring:!1,sessionCreating:!1,pageCreating:!1,reviewLoading:!1,dropdownOpen:!1,codexAccessSelectOpen:!1,project:null,page:null,providers:[],currentUser:null,providerId:`codex`,prompt:``,includeProjectContext:!0,includePageContext:!0,error:``,reviewSessionId:``,reviewData:null,localBridge:{url:r,checked:!1,checking:!1,available:!1,version:``,mode:``,error:``}};function l(e){return String(e||``).replaceAll(`&`,`&amp;`).replaceAll(`<`,`&lt;`).replaceAll(`>`,`&gt;`).replaceAll(`"`,`&quot;`).replaceAll(`'`,`&#39;`)}function u(){return c.providers.find(e=>e.id===c.providerId)||c.providers[0]||null}function d(e=u()){return String(e?.availableVia||``).trim().toLowerCase()===`local-bridge`}function f(){if(c.currentUser)return c.currentUser;if(window.uxBridgeUser)return c.currentUser=window.uxBridgeUser,c.currentUser;try{let e=sessionStorage.getItem(`ux-bridge-user`);c.currentUser=e?JSON.parse(e):null}catch{c.currentUser=null}return c.currentUser}function p(e=c.providerId){let t=c.providers.find(t=>t.id===e)||null;return d(t)?!!c.localBridge.available:t?.isConfigured?!0:!!f()?.integrations?.[e]?.connected}function m(){return document.querySelector(`[data-vibe-mobile-render]`)||document.querySelector(`.vibe-mobile-stage`)||null}function h(){return m()?.querySelector?.(`.vibe-generated-page`)||document.querySelector(`[data-vibe-mobile-render] .vibe-generated-page`)||document.querySelector(`.vibe-generated-page`)||null}function g(){let e=h();return e?e.matches?.(`[data-ux-layer-selected]:not([data-ux-layer-selected="false"])`)?e:e.querySelector(`[data-ux-layer-selected]:not([data-ux-layer-selected="false"])`):null}function _(e){let t=m();if(!t||!e||!t.contains(e))return``;let n=[],r=e;for(;r&&r!==t;){let e=r.parentElement;if(!e)break;n.unshift(Array.from(e.children).indexOf(r)),r=e}return n.filter(e=>e>=0).join(`.`)}function v(e,t){let n=String(e||``);return n.length>t?n.slice(0,t):n}function y(){let e=c.page?.preview||c.page?.vibe?.appliedDraft||{};return{css:v(e.css||``,2e4),js:v(e.js||``,12e3),breakpointOverrides:e.breakpointOverrides||{}}}function b(e){if(!e)return``;let t=e.tagName?e.tagName.toLowerCase():`element`,n=typeof e.className==`string`?e.className.trim().replace(/\s+/g,`.`):``,r=e.textContent?e.textContent.replace(/\s+/g,` `).trim().slice(0,80):``;return[n?`${t}.${n}`:t,r?`"${r}"`:``].filter(Boolean).join(` `)}function x(){let e=g();return e?{type:`selected-layer`,layerPath:_(e),label:b(e),html:v(e.outerHTML||``,7e4),text:v(e.textContent||``,4e3),childCount:e.querySelectorAll?e.querySelectorAll(`*`).length:0,currentPreview:y()}:null}async function S(e=!1){if(!(c.localBridge.checking||!e&&c.localBridge.checked)){c.localBridge.checking=!0,B();try{let e=await fetch(`${c.localBridge.url}/health`,{method:`GET`,cache:`no-store`}),t=await e.json().catch(()=>({}));c.localBridge.checked=!0,c.localBridge.available=!!(e.ok&&t?.ok),c.localBridge.version=String(t?.version||``).trim(),c.localBridge.mode=String(t?.mode||``).trim(),c.localBridge.error=c.localBridge.available?``:`The local Codex bridge did not respond with a healthy status.`}catch{c.localBridge.checked=!0,c.localBridge.available=!1,c.localBridge.version=``,c.localBridge.mode=``,c.localBridge.error=`Start the local Codex bridge on this machine to use Codex App for page generation.`}finally{c.localBridge.checking=!1,B()}}}function C(e,t,n){return d(e)?c.localBridge.checking?{tone:`neutral`,text:`Checking for a local Codex bridge on this machine…`}:t?{tone:`connected`,text:`Codex App bridge is ready${c.localBridge.version?` (v${l(c.localBridge.version)})`:``}.`}:{tone:`neutral`,text:c.localBridge.error||`Start the local Codex bridge on this machine to use Codex App for page generation.`}:t?{tone:`connected`,text:`${e?.label||`Provider`} is connected${n?` as ${n}`:``}.`}:{tone:`neutral`,text:`Connect ${e?.label||`this provider`} in Profile before generating.`}}function w(){document.body.classList.toggle(`bridge-body--has-vibe`,!0),document.body.classList.toggle(`vibe-open`,c.drawerOpen),e.inert=!c.drawerOpen;let t=document.querySelector(`[data-vibe-drawer-toggle]`);t&&(t.setAttribute(`aria-expanded`,c.drawerOpen?`true`:`false`),t.classList.toggle(`is-active`,c.drawerOpen)),T()}function T(){if(!n)return;let e=window.innerWidth<=959,t=document.body.classList.contains(`customizer-open`)||document.body.classList.contains(`comments-open`)||document.body.classList.contains(`uploads-open`)||document.body.classList.contains(`vibe-open`),r=e&&t;n.hidden=!r,n.classList.toggle(`is-visible`,r)}function E(e,t=`vibe`){c.drawerOpen!==e&&(c.drawerOpen=e,w(),e?window.dispatchEvent(new CustomEvent(`uxbridge:drawer-open`,{detail:{drawer:t}})):(c.dropdownOpen=!1,B()))}function D(){let e=window.UXBridgeActionRail?.renderButtonContent||(({label:e,tooltipClass:t=``}={})=>`
      ...
      `),t=document.querySelector(`[data-side-actions]`),n=document.createElement(`button`);n.type=`button`,n.className=`bridge-action-rail-button vibe-drawer-toggle`,n.setAttribute(`data-vibe-drawer-toggle`,``),n.setAttribute(`aria-expanded`,`false`),n.setAttribute(`aria-label`,`Vibe code`),n.innerHTML=e({icon:`vibe`,label:`Vibe code`,tooltipClass:`vibe-drawer-toggle__tooltip`}),n.addEventListener(`click`,()=>{E(!c.drawerOpen)}),window.addEventListener(`uxbridge:drawer-open`,e=>{e.detail?.drawer!==`vibe`&&E(!1,`vibe`)}),t&&t.prepend(n),w()}function O(e,t=s||c.page?.id){if(!e||(c.project=e,c.page=e.pages.find(e=>e.id===t)||e.pages[0]||null,!c.page))return;let n=c.page.vibe||{};c.providerId=n.providerId||c.providerId,c.prompt=n.prompt||c.prompt,c.includeProjectContext=n.includeProjectContext!==!1,c.includePageContext=n.includePageContext!==!1,c.reviewSessionId&&!(Array.isArray(e.editSessions)?e.editSessions:[]).some(e=>e.id===c.reviewSessionId)&&(c.reviewSessionId=``,c.reviewData=null),B()}function k(){return String(f()?.email||``).trim().toLowerCase()}function A(){let e=k();return!e||!c.project?null:(Array.isArray(c.project.editSessions)?c.project.editSessions:[]).find(t=>{let n=String(t.status||``).trim().toLowerCase();return(n===`active`||n===`ready_for_review`)&&String(t.userId||``).trim().toLowerCase()===e&&String(t.pageId||``).trim().toLowerCase()===String(c.page?.id||``).trim().toLowerCase()})||null}function j(){return!c.project||!c.page?[]:(Array.isArray(c.project.editSessions)?c.project.editSessions:[]).filter(e=>String(e.pageId||``).trim().toLowerCase()===String(c.page.id||``).trim().toLowerCase()).sort((e,t)=>{let n=Number(e.updatedAt||e.createdAt||0);return Number(t.updatedAt||t.createdAt||0)-n})}function M(){return!!(c.project?.canManageSharing||c.project?.isOwner||c.project?.projectRole===`admin`)}function N(e){if(!c.project||!e)return null;let t=String(e.userId||``).trim().toLowerCase();return[...Array.isArray(c.project.projectMembers)?c.project.projectMembers:[],...Array.isArray(c.project.availableUsers)?c.project.availableUsers:[]].find(e=>String(e?.email||``).trim().toLowerCase()===t)||null}function P(e){let t=k(),n=String(e?.userId||``).trim().toLowerCase(),r=N(e);return t&&n===t?`You`:r?.fullName||e?.userId||`Project member`}function ee(e){let t=String(e||``).split(`
`).map(e=>e.trim()).find(Boolean);return t?t.replace(/^create\s+/i,``).replace(/^build\s+/i,``).replace(/^design\s+/i,``).replace(/^a\s+/i,``).replace(/^an\s+/i,``).slice(0,48).trim()||`New Page`:``}function F(){return c.providers.map(e=>{let t=e.id===c.providerId;return`
          <button
            type="button"
        ...
        `}).join(``)}function I(e){return i.find(t=>t.value===e)?.label||i[1].label}function L(){let e=c.page?.vibe,t=e?.lastDraft,n=e?.appliedDraft,r=Array.isArray(e?.draftHistory)?e.draftHistory:[];return t?`
    ...
    `:`
      ...
      `}function R(){return!c.reviewSessionId||!c.project?null:(Array.isArray(c.project.editSessions)?c.project.editSessions:[]).find(e=>e.id===c.reviewSessionId)||null}function z(){let e=R();if(!e)return``;let t=c.reviewData,n=c.project?.pages?.find(t=>t.id===e.pageId);return`
      <section class="vibe-panel__review-card vibe-panel__review-card--detail">
        ...
        ${M()?`<div class="vibe-panel__result-actions"><button type="button" class="vibe-panel__button vibe-panel__button--secondary" data-vibe-merge-session="${l(e.id)}" ${c.applying?`disabled`:``}>${c.applying?`Merging...`:`Merge session`}</button></div>`:``}
      </section>
    `}function B(){let t=u(),n=p(t?.id),r=C(t,n,f()?.integrations?.[t?.id||``]?.accountLabel||``),a=c.page?.name||`Page`,o=c.project?.name||`Project`,s=c.page?.vibe||{},m=A(),h=Array.isArray(c.project?.editSessions)?c.project.editSessions:[],g=j(),_=h.filter(e=>String(e.status||``).trim().toLowerCase()===`ready_for_review`).sort((e,t)=>Number(t.updatedAt||t.createdAt||0)-Number(e.updatedAt||e.createdAt||0)),v=g.filter(e=>String(e.status||``).trim().toLowerCase()===`ready_for_review`),y=g.filter(e=>String(e.status||``).trim().toLowerCase()===`active`),b=!!(m||c.project?.canCreateEditSession),x=String(m?.status||``).trim().toLowerCase(),S=!!(c.project?.canCreateEditSession&&c.prompt.trim()&&x!==`ready_for_review`),w=x?x===`active`:!!b,T=M();e.innerHTML=`
      <div class="vibe-panel__inner">
        ...
        <section class="vibe-panel__section">
                ...
                ${c.project?.canManageSharing?`
                      <div class="vibe-panel__field vibe-panel__field--compact">
                        ...
                        <div class="vibe-panel__provider-select">
                          ...
                          >
                            <span>
                              <strong>${l(I(c.project?.codexAccessMode||`contributors`))}</strong>
              ...
              <div class="vibe-panel__session-card${m?` is-active`:``}">
                ...
                ${!m&&c.project?.canCreateEditSession?`<button type="button" class="vibe-panel__button vibe-panel__button--ghost" data-vibe-create-session ${c.sessionCreating?`disabled`:``}>${c.sessionCreating?`Starting...`:`Start session`}</button>`:m?`
                        <div class="vibe-panel__session-actions">
                          ...
                          ${x===`ready_for_review`&&T?`<button type="button" class="vibe-panel__button vibe-panel__button--secondary" data-vibe-merge-session="${l(m.id)}">Merge session</button>`:``}
                        </div>
                      `:``}
              </div>
              ${g.length?`
          ...
          <div class="vibe-panel__stack">
            <button type="button" class="vibe-panel__button" data-vibe-generate ${c.loading||!n||!w?`disabled`:``}>
    ...
    `,V()}function V(){e.querySelector(`[data-vibe-close]`)?.addEventListener(`click`,()=>{E(!1)}),e.querySelector(`[data-vibe-provider-toggle]`)?.addEventListener(`click`,()=>{c.dropdownOpen=!c.dropdownOpen,c.codexAccessSelectOpen=!1,B()}),e.querySelector(`[data-vibe-codex-access-toggle]`)?.addEventListener(`click`,()=>{c.codexAccessSelectOpen=!c.codexAccessSelectOpen,c.dropdownOpen=!1,B()}),e.querySelectorAll(`[data-vibe-provider-option]`).forEach(e=>{e.addEventListener(`click`,()=>{c.providerId=e.dataset.vibeProviderOption||c.providerId,c.dropdownOpen=!1,B(),d()&&S(!0)})}),e.querySelectorAll(`[data-vibe-codex-access-option]`).forEach(e=>{e.addEventListener(`click`,()=>{G(e.getAttribute(`data-vibe-codex-access-option`)||``)})}),e.querySelector(`[data-vibe-prompt]`)?.addEventListener(`input`,e=>{c.prompt=e.currentTarget.value}),e.querySelectorAll(`[data-vibe-context]`).forEach(t=>{t.addEventListener(`change`,()=>{c.includeProjectContext=e.querySelector(`[data-vibe-context="project"]`)?.checked!==!1,c.includePageContext=e.querySelector(`[data-vibe-context="page"]`)?.checked!==!1})}),e.querySelector(`[data-vibe-generate]`)?.addEventListener(`click`,()=>{X()}),e.querySelector(`[data-vibe-create-session]`)?.addEventListener(`click`,()=>{U()}),e.querySelector(`[data-vibe-create-page]`)?.addEventListener(`click`,()=>{W()}),e.querySelectorAll(`[data-vibe-session-status]`).forEach(e=>{e.addEventListener(`click`,()=>{K(e.getAttribute(`data-vibe-session-status`)||``)})}),e.querySelectorAll(`[data-vibe-merge-session]`).forEach(e=>{e.addEventListener(`click`,()=>{q(e.getAttribute(`data-vibe-merge-session`)||``)})}),e.querySelectorAll(`[data-vibe-open-review]`).forEach(e=>{e.addEventListener(`click`,()=>{J(e.getAttribute(`data-vibe-open-review`)||``)})}),e.querySelector(`[data-vibe-close-review]`)?.addEventListener(`click`,()=>{c.reviewSessionId=``,c.reviewData=null,B()}),e.querySelector(`[data-vibe-apply]`)?.addEventListener(`click`,()=>{Z()}),e.querySelectorAll(`[data-vibe-restore]`).forEach(e=>{e.addEventListener(`click`,()=>{Q(e.getAttribute(`data-vibe-restore`))})}),e.querySelectorAll(`[data-vibe-apply-version]`).forEach(e=>{e.addEventListener(`click`,()=>{Z(e.getAttribute(`data-vibe-apply-version`))})})}async function H(e){let n=x(),r=await fetch(t,{method:`POST`,credentials:`include`,headers:{"Content-Type":`application/json`},body:JSON.stringify({action:`saveVibeDraft`,project:c.project.id,page:c.page.id,providerId:c.providerId,prompt:c.prompt,includeProjectContext:c.includeProjectContext,includePageContext:c.includePageContext,scope:n,generated:e})}),i=await r.json().catch(()=>({}));if(!r.ok||!i?.ok||!i?.project)throw Error(i?.error||`Unable to save the generated page draft.`);c.providers=Array.isArray(i.vibeProviders)&&i.vibeProviders.length?i.vibeProviders:c.providers,O(i.project,c.page.id),window.dispatchEvent(new CustomEvent(`uxbridge:project-runtime-sync`,{detail:{project:i.project}}))}async function U(){if(!c.project||!c.page)return null;let e=A();if(e)return e;if(!c.project.canCreateEditSession||c.sessionCreating)return null;c.sessionCreating=!0,c.error=``,B();try{let e=await fetch(t,{method:`POST`,credentials:`include`,headers:{"Content-Type":`application/json`},body:JSON.stringify({action:`createEditSession`,project:c.project.id,page:c.page.id,source:d()?`local-bridge`:c.providerId})}),n=await e.json().catch(()=>({}));if(!e.ok||!n?.ok||!n?.project)throw Error(n?.error||`Unable to start a Codex edit session for this page.`);return O(n.project,c.page.id),window.dispatchEvent(new CustomEvent(`uxbridge:project-runtime-sync`,{detail:{project:n.project}})),n.session||A()}catch(e){return c.error=e instanceof Error?e.message:`Unable to start a Codex edit session for this page.`,B(),null}finally{c.sessionCreating=!1,B()}}async function W(){if(!(!c.project||c.pageCreating||!c.prompt.trim())&&await U()){c.pageCreating=!0,c.error=``,B();try{let e=await fetch(t,{method:`POST`,credentials:`include`,headers:{"Content-Type":`application/json`},body:JSON.stringify({action:`createPageFromCodex`,project:c.project.id,name:ee(c.prompt)})}),n=await e.json().catch(()=>({}));if(!e.ok||!n?.ok||!n?.page?.launchUrl)throw Error(n?.error||`Unable to create a new page from this prompt.`);window.location.href=n.page.launchUrl}catch(e){c.error=e instanceof Error?e.message:`Unable to create a new page from this prompt.`,B()}finally{c.pageCreating=!1,B()}}}async function G(e=``){if(!(!c.project||!e)){c.sessionCreating=!0,c.error=``,c.codexAccessSelectOpen=!1,B();try{let n=await fetch(t,{method:`POST`,credentials:`include`,headers:{"Content-Type":`application/json`},body:JSON.stringify({action:`updateProjectPrivacy`,project:c.project.id,codexAccessMode:e})}),r=await n.json().catch(()=>({}));if(!n.ok||!r?.ok||!r?.project)throw Error(r?.error||`Unable to update Codex access settings.`);O(r.project,c.page.id),window.dispatchEvent(new CustomEvent(`uxbridge:project-runtime-sync`,{detail:{project:r.project}}))}catch(e){c.error=e instanceof Error?e.message:`Unable to update Codex access settings.`,B()}finally{c.sessionCreating=!1,B()}}}async function K(e=``){let n=A();if(!(!c.project||!n||!e)){c.sessionCreating=!0,c.error=``,B();try{let r=await fetch(t,{method:`POST`,credentials:`include`,headers:{"Content-Type":`application/json`},body:JSON.stringify({action:`updateEditSessionStatus`,project:c.project.id,sessionId:n.id,status:e})}),i=await r.json().catch(()=>({}));if(!r.ok||!i?.ok||!i?.project)throw Error(i?.error||`Unable to update the edit session status.`);O(i.project,c.page.id),window.dispatchEvent(new CustomEvent(`uxbridge:project-runtime-sync`,{detail:{project:i.project}}))}catch(e){c.error=e instanceof Error?e.message:`Unable to update the edit session status.`,B()}finally{c.sessionCreating=!1,B()}}}async function q(e=``){if(!(!c.project||!e||c.applying)){c.applying=!0,c.error=``,B();try{let n=await fetch(t,{method:`POST`,credentials:`include`,headers:{"Content-Type":`application/json`},body:JSON.stringify({action:`mergeEditSession`,project:c.project.id,sessionId:e})}),r=await n.json().catch(()=>({}));if(!n.ok||!r?.ok||!r?.project)throw Error(r?.error||`Unable to merge this edit session.`);c.reviewSessionId===e&&(c.reviewSessionId=``,c.reviewData=null),O(r.project,c.page.id),window.dispatchEvent(new CustomEvent(`uxbridge:project-runtime-sync`,{detail:{project:r.project}}))}catch(e){c.error=e instanceof Error?e.message:`Unable to merge this edit session.`,B()}finally{c.applying=!1,B()}}}async function J(e=``){if(!(!c.project||!e||c.reviewLoading)){c.reviewSessionId=e,c.reviewData=null,c.reviewLoading=!0,c.error=``,B();try{let n=await fetch(t,{method:`POST`,credentials:`include`,headers:{"Content-Type":`application/json`},body:JSON.stringify({action:`getEditSessionReview`,project:c.project.id,sessionId:e})}),r=await n.json().catch(()=>({}));if(!n.ok||!r?.ok||!r?.review)throw Error(r?.error||`Unable to load the edit-session review.`);c.reviewData=r.review}catch(e){c.error=e instanceof Error?e.message:`Unable to load the edit-session review.`}finally{c.reviewLoading=!1,B()}}}async function Y(){let e=x(),t=await fetch(`${c.localBridge.url}/v1/generate-page`,{method:`POST`,headers:{"Content-Type":`application/json`},body:JSON.stringify({projectId:c.project.id,projectName:c.project.name,pageId:c.page.id,pageName:c.page.name,prompt:c.prompt,includeProjectContext:c.includeProjectContext,includePageContext:c.includePageContext,scope:e})}),n=await t.json().catch(()=>({}));if(!t.ok||!n?.ok||!n?.result)throw Error(n?.error||`The local Codex bridge could not generate page content.`);await H(n.result)}async function X(){if(!(!c.project||!c.page||!c.prompt.trim()||c.loading)){if(!p()){c.error=`Connect ${u()?.label||`this provider`} in your Profile before generating.`,B();return}c.loading=!0,c.error=``,B();try{if(!await U())return;if(d()){await Y();return}let e=await fetch(t,{method:`POST`,credentials:`include`,headers:{"Content-Type":`application/json`},body:JSON.stringify({action:`generateVibeContent`,project:c.project.id,page:c.page.id,providerId:c.providerId,prompt:c.prompt,includeProjectContext:c.includeProjectContext,includePageContext:c.includePageContext,scope:x()})}),n=await e.json().catch(()=>({}));if(!e.ok||!n?.ok||!n?.project)throw Error(n?.error||`Unable to generate page content.`);c.providers=Array.isArray(n.vibeProviders)&&n.vibeProviders.length?n.vibeProviders:c.providers,O(n.project,c.page.id),window.dispatchEvent(new CustomEvent(`uxbridge:project-runtime-sync`,{detail:{project:n.project}}))}catch(e){c.error=e instanceof Error?e.message:`Unable to generate page content.`,B()}finally{c.loading=!1,B()}}}async function Z(e=``){if(!(!c.project||!c.page||c.applying)){c.applying=!0,c.error=``,B();try{let n=await fetch(t,{method:`POST`,credentials:`include`,headers:{"Content-Type":`application/json`},body:JSON.stringify({action:`applyVibeContent`,project:c.project.id,page:c.page.id,...e?{draftGeneratedAt:Number(e)||0}:{}})}),r=await n.json().catch(()=>({}));if(!n.ok||!r?.ok||!r?.project)throw Error(r?.error||`Unable to apply generated page content.`);c.providers=Array.isArray(r.vibeProviders)&&r.vibeProviders.length?r.vibeProviders:c.providers,O(r.project,c.page.id),window.dispatchEvent(new CustomEvent(`uxbridge:project-runtime-sync`,{detail:{project:r.project}}))}catch(e){c.error=e instanceof Error?e.message:`Unable to apply generated page content.`,B()}finally{c.applying=!1,B()}}}async function Q(e=``){if(!(!c.project||!c.page||c.restoring)){c.restoring=!0,c.error=``,B();try{let n=await fetch(t,{method:`POST`,credentials:`include`,headers:{"Content-Type":`application/json`},body:JSON.stringify({action:`restoreVibeDraft`,project:c.project.id,page:c.page.id,draftGeneratedAt:Number(e)||0})}),r=await n.json().catch(()=>({}));if(!n.ok||!r?.ok||!r?.project)throw Error(r?.error||`Unable to restore this version.`);c.providers=Array.isArray(r.vibeProviders)&&r.vibeProviders.length?r.vibeProviders:c.providers,O(r.project,c.page.id),window.dispatchEvent(new CustomEvent(`uxbridge:project-runtime-sync`,{detail:{project:r.project}}))}catch(e){c.error=e instanceof Error?e.message:`Unable to restore this version.`,B()}finally{c.restoring=!1,B()}}}async function te(){try{let e=await fetch(`${t}?project=${encodeURIComponent(o)}`,{credentials:`include`,cache:`no-store`}),n=await e.json().catch(()=>({}));if(!e.ok||!n?.ok||!n?.project)return;c.providers=Array.isArray(n.vibeProviders)&&n.vibeProviders.length?n.vibeProviders:$(),c.currentUser=n.currentUser||f(),O(n.project,s),S()}catch{c.providers=$(),B()}}function $(){return[{id:`codex-app`,label:`Codex App`,availableVia:`local-bridge`,helperCopy:`Uses a local Codex bridge on this machine to generate page-scoped mobile UI.`},{id:`codex`,label:`Codex`,helperCopy:`Ready for a user-owned Codex session or connector-based execution flow.`},{id:`claude`,label:`Claude`,helperCopy:`Structured for connector-based Claude execution with the user’s own credentials.`},{id:`generic`,label:`Other tool`,helperCopy:`Fallback adapter for other vibe-coding tools while preserving the same UX Bridge contract.`}]}document.addEventListener(`click`,t=>{c.dropdownOpen&&(e.contains(t.target)||(c.dropdownOpen=!1,B()))}),document.addEventListener(`keydown`,e=>{if(e.key===`Escape`){if(c.dropdownOpen){c.dropdownOpen=!1,B();return}c.drawerOpen&&E(!1)}}),window.addEventListener(`uxbridge:project-page-sync`,e=>{let t=e.detail?.project,n=e.detail?.page;!t||!n||t.id!==o||(c.providers.length||(c.providers=$()),O(t,n.id))}),window.addEventListener(`uxbridge:user-ready`,e=>{c.currentUser=e.detail||null,B()}),c.providers=$(),D(),B(),n?.addEventListener(`click`,()=>{c.drawerOpen&&E(!1)}),window.addEventListener(`resize`,T),document.querySelector(`[data-vibe-launch]`)?.addEventListener(`click`,()=>{E(!0)}),te()})();