...
export async function handleProjectsRequest(req) {
  ...
  if (action === "savePreviewContentEdits") {
    ...
    const nextPreview = normalizePagePreview({
      ...
      html: syncBase ? fallbackHtml : String(currentPreview?.html || ""),
      css: syncBase ? fallbackCss : String(currentPreview?.css || ""),
      js: syncBase ? (hasPayloadJs ? fallbackJs : currentPreview?.js || "") : String(currentPreview?.js || ""),
      stageStyle: syncBase ? fallbackStageStyle : String(currentPreview?.stageStyle || "").trim(),
